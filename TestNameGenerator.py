...
class GenerateTestFilesCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        ...
        settings = sublime.load_settings('TestNameGenerator.sublime-settings')

        itemMethodExtractor                         = MethodExtractor()
        itemMethodExtractor.FLAG_manualPath         = settings.get('generate_test_flag')
        itemMethodExtractor.PATH_basePathRepository = settings.get('generate_test_base_path')
        itemMethodExtractor.PATH_sourceRepository   = settings.get('generate_test_secure_path')
        itemMethodExtractor.PATH_testRepository     = settings.get('generate_test_tests_path')

        if False == self.isAllowedSyntax(currentSyntax):
            return

        try:
            itemMethodExtractor.allFilesList.append(self.view.file_name())
        except ValueError:
            pass
...
class MethodExtractor():
    ...
    PATH_basePathRepository       = "/home/bogdan/workspace/project/"
    PATH_logOutput                = 'logs';
    PATH_sourceRepository         = "source"
    PATH_testRepository           = "tests"
